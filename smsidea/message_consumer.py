...
class KafkaMessageConsumerThread(threading.Thread):
    def __new__(cls):
        if not hasattr(cls, 'instance'):
            cls.instance = super(KafkaMessageConsumerThread, cls).__new__(cls)
        return cls.instance
    ...
    def __init__(self):
        ...
        self.consumer = Consumer({
            'bootstrap.servers': config.get('message.consumer', 'bootstrap_servers'),
            'group.id': config.get('message.consumer', 'group_id'),
            'auto.offset.reset': 'earliest',
            'enable.auto.commit': False  # Disable auto-commit to manage offset manually
        })

        self.topic = config.get('message.consumer', 'topic')
